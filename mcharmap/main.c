...
main (int argc, char **argv)
{
  ...
  mcharmap_settings_initialize ();

  g_set_application_name (_("Character Map"));
  gtk_window_set_default_icon_name (MCHARMAP_ICON_NAME);

  window = mcharmap_window_new ();
  ...
  if (!font)
    font = mcharmap_settings_get_font ();

  if (font)
    {
      mcharmap_window_set_font (MCHARMAP_WINDOW (window), font);
      g_free (font);
    }

  mcharmap_settings_add_window (GTK_WINDOW (window));

  gtk_window_present (GTK_WINDOW (window));

  gtk_main ();

  mcharmap_settings_shutdown ();
