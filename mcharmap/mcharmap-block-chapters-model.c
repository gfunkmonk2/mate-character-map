...
enum 
{
  BLOCK_CHAPTERS_MODEL_ID = MCHARMAP_CHAPTERS_MODEL_COLUMN_ID,
  BLOCK_CHAPTERS_MODEL_LABEL = MCHARMAP_CHAPTERS_MODEL_COLUMN_LABEL,
...
static void
mcharmap_block_chapters_model_init (McharmapBlockChaptersModel *model)
{
                    ...
                    GtkTreeIter *iter)
{
  ...
  return mcharmap_block_codepoint_list_new (unicode_block->start, unicode_block->end);
}

static McharmapCodepointList *
get_book_codepoint_list (McharmapChaptersModel *model)
{
  McharmapChaptersModelPrivate *model_priv = model->priv;

  if (model_priv->book_list == NULL) {
    model_priv->book_list = mcharmap_block_codepoint_list_new (0, UNICHAR_MAX);
  }
...
/* XXX linear search */
static gboolean
character_to_iter (McharmapChaptersModel *chapters,
                   gunichar           wc,
                   GtkTreeIter       *_iter)
{
  GtkTreeModel *model = GTK_TREE_MODEL (chapters);
...
static void
mcharmap_block_chapters_model_class_init (McharmapBlockChaptersModelClass *clazz)
{
...
G_DEFINE_TYPE (McharmapBlockChaptersModel, mcharmap_block_chapters_model, MCHARMAP_TYPE_CHAPTERS_MODEL)

McharmapChaptersModel *
mcharmap_block_chapters_model_new (void)
{
  return g_object_new (mcharmap_block_chapters_model_get_type (), NULL);
}
