...
struct _McharmapCharmapPrivate {
  ...
  GtkWidget *notebook;
  McharmapChaptersView *chapters_view;
...
static guint mcharmap_charmap_signals[NUM_SIGNALS];

static void mcharmap_charmap_class_init (McharmapCharmapClass *klass);
static void mcharmap_charmap_init       (McharmapCharmap *charmap);

G_DEFINE_TYPE (McharmapCharmap, mcharmap_charmap, GTK_TYPE_PANED)

static void
mcharmap_charmap_finalize (GObject *object)
{
  McharmapCharmap *charmap = MCHARMAP_CHARMAP (object);
  McharmapCharmapPrivate *priv = charmap->priv;
  ...
  G_OBJECT_CLASS (mcharmap_charmap_parent_class)->finalize (object);
}

static void
mcharmap_charmap_get_property (GObject *object,
                                guint prop_id,
                                GValue *value,
                                GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_ORIENTATION:
      g_value_set_enum (value, mcharmap_charmap_get_orientation (charmap));
      break;
#endif
    case PROP_CHAPTERS_MODEL:
      g_value_set_object (value, mcharmap_charmap_get_chapters_model (charmap));
      break;
    case PROP_ACTIVE_CHAPTER:
      g_value_take_string (value, mcharmap_chapters_view_get_selected (priv->chapters_view));
      break;
    case PROP_ACTIVE_CHARACTER:
      g_value_set_uint (value, mcharmap_charmap_get_active_character (charmap));
      break;
    case PROP_ACTIVE_CODEPOINT_LIST:
      g_value_set_object (value, mcharmap_charmap_get_active_codepoint_list (charmap));
      break;
    case PROP_ACTIVE_PAGE:
      g_value_set_uint (value, mcharmap_charmap_get_active_page (charmap));
      break;
    case PROP_FONT_DESC:
      g_value_set_boxed (value, mcharmap_charmap_get_font_desc (charmap));
      break;
    case PROP_SNAP_POW2:
      g_value_set_boolean (value, mcharmap_charmap_get_snap_pow2 (charmap));
      break;
    default:
...
static void
mcharmap_charmap_set_property (GObject *object,
                                guint prop_id,
                                const GValue *value,
                                GParamSpec *pspec)
{
...
#if !GTK_CHECK_VERSION (2, 15, 0)
    case PROP_ORIENTATION:
      mcharmap_charmap_set_orientation (charmap, g_value_get_enum (value));
      break;
#endif
    case PROP_CHAPTERS_MODEL:
      mcharmap_charmap_set_chapters_model (charmap, g_value_get_object (value));
      break;
    case PROP_ACTIVE_CHAPTER:
      mcharmap_chapters_view_set_selected (priv->chapters_view,
                                            g_value_get_string (value));
      break;
    case PROP_ACTIVE_CHARACTER:
      mcharmap_charmap_set_active_character (charmap, g_value_get_uint (value));
      break;
    case PROP_ACTIVE_PAGE:
      mcharmap_charmap_set_active_page (charmap, g_value_get_uint (value));
      break;
    case PROP_FONT_DESC:
      mcharmap_charmap_set_font_desc (charmap, g_value_get_boxed (value));
      break;
    case PROP_SNAP_POW2:
      mcharmap_charmap_set_snap_pow2 (charmap, g_value_get_boolean (value));
      break;
...
static void
mcharmap_charmap_size_request (GtkWidget *widget,
                                GtkRequisition *requisition)
{
  McharmapCharmap *charmap = MCHARMAP_CHARMAP (widget);
  McharmapCharmapPrivate *priv = charmap->priv;
...
static void
mcharmap_charmap_size_allocate (GtkWidget *widget,
...
static void
mcharmap_charmap_class_init (McharmapCharmapClass *klass)
{
  ...
  object_class->get_property = mcharmap_charmap_get_property;
  object_class->set_property = mcharmap_charmap_set_property;
  object_class->finalize = mcharmap_charmap_finalize;

#if !GTK_CHECK_VERSION (2, 15, 0)
  widget_class->size_request = mcharmap_charmap_size_request;
  widget_class->size_allocate = mcharmap_charmap_size_allocate;
#endif

  mcharmap_charmap_signals[STATUS_MESSAGE] =
      g_signal_new (I_("status-message"), mcharmap_charmap_get_type (),
                    G_SIGNAL_RUN_FIRST,
                    G_STRUCT_OFFSET (McharmapCharmapClass, status_message),
                    NULL, NULL, g_cclosure_marshal_VOID__STRING, G_TYPE_NONE,
                    1, G_TYPE_STRING);

  mcharmap_charmap_signals[LINK_CLICKED] =
      g_signal_new (I_("link-clicked"), mcharmap_charmap_get_type (),
                    G_SIGNAL_RUN_FIRST,
                    G_STRUCT_OFFSET (McharmapCharmapClass, link_clicked),
                    NULL, NULL, _mcharmap_marshal_VOID__UINT_UINT, G_TYPE_NONE, 
  ...
  g_type_class_add_private (object_class, sizeof (McharmapCharmapPrivate));
}

static void
mcharmap_charmap_update_text_tags (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;
  GtkStyle *style;
...
static void
mcharmap_charmap_set_font_desc_internal (McharmapCharmap *charmap, 
                                          ...
                                          gboolean in_notification)
{
  ...
  if (!in_notification)
    mcharmap_chartable_set_font_desc (priv->chartable, font_desc);

  if (gtk_widget_get_style (GTK_WIDGET (priv->details_view)))
    mcharmap_charmap_update_text_tags (charmap);
...
static void
insert_vanilla_detail (McharmapCharmap *charmap, 
...
static void
insert_codepoint (McharmapCharmap *charmap,
                  GtkTextBuffer *buffer,
                  GtkTextIter *iter,
                  gunichar uc)
{
...
static void
insert_chocolate_detail_codepoints (McharmapCharmap *charmap,
                                    GtkTextBuffer *buffer,
                                    GtkTextIter *iter,
                                    const gchar *name,
                                    const gunichar *ucs)
...
static void
insert_string_link_codepoints (McharmapCharmap *charmap,
                               GtkTextBuffer *buffer,
                               GtkTextIter *iter,
                               const gchar *str)
{
...
static void
insert_chocolate_detail (McharmapCharmap *charmap,
                         GtkTextBuffer *buffer,
                         GtkTextIter *iter,
                         const gchar *name,
                         const gchar **values,
...
static void
insert_heading (McharmapCharmap *charmap, 
                GtkTextBuffer *buffer,
                GtkTextIter *iter,
                const gchar *heading)
{
...
static void
conditionally_insert_canonical_decomposition (McharmapCharmap *charmap, 
                                              GtkTextBuffer *buffer,
                                              GtkTextIter *iter,
                                              gunichar uc)
{
...
static void
set_details (McharmapCharmap *charmap,
             gunichar uc)
{
  McharmapCharmapPrivate *priv = charmap->priv;
  GtkTextBuffer *buffer;
  ...
  n = mcharmap_unichar_to_printable_utf8 (uc, buf);
  if (n == 0)
  ...
  temp = g_strdup_printf ("U+%4.4X %s\n", 
                          uc, mcharmap_get_unicode_name (uc));
  ...
  /* Unicode version */
  version = mcharmap_get_unicode_version (uc);
  if (version != MCHARMAP_UNICODE_VERSION_UNASSIGNED)
    insert_vanilla_detail (charmap, buffer, &iter,
                           _("In Unicode since:"),
                           mcharmap_unicode_version_to_string (version));

  /* character category */
  insert_vanilla_detail (charmap, buffer, &iter, _("Unicode category:"),
                         mcharmap_get_unicode_category_name (uc));
  ...
  if (_mcharmap_unicode_has_nameslist_entry (uc))
    {
      ...
      csarr = mcharmap_get_nameslist_stars (uc);
      if (csarr != NULL)
        {
          insert_chocolate_detail (charmap, buffer, &iter,
                                   _("Notes:"), csarr, TRUE);
      ...
      ucs = mcharmap_get_nameslist_exes (uc);
      if (ucs != NULL)
        {
      ...
      csarr = mcharmap_get_nameslist_pounds (uc);
      if (csarr != NULL)
        {
          insert_chocolate_detail (charmap, buffer, &iter,
      ...
      csarr = mcharmap_get_nameslist_colons (uc);
      if (csarr != NULL)
        {
          insert_chocolate_detail (charmap, buffer, &iter,
  ...
  if (mcharmap_get_unicode_kDefinition (uc)
      || mcharmap_get_unicode_kCantonese (uc)
      || mcharmap_get_unicode_kMandarin (uc)
      || mcharmap_get_unicode_kJapaneseOn (uc)
      || mcharmap_get_unicode_kJapaneseKun (uc)
      || mcharmap_get_unicode_kTang (uc)
      || mcharmap_get_unicode_kKorean (uc))
    {
      insert_heading (charmap, buffer, &iter, _("CJK Ideograph Information"));

      csp = mcharmap_get_unicode_kDefinition (uc);
      if (csp)
        insert_vanilla_detail (charmap, buffer, &iter,
                               _("Definition in English:"), csp);
    
      csp = mcharmap_get_unicode_kMandarin (uc);
      if (csp)
        insert_vanilla_detail (charmap, buffer, &iter,
                               _("Mandarin Pronunciation:"), csp);
    
      csp = mcharmap_get_unicode_kCantonese (uc);
      if (csp)
        insert_vanilla_detail (charmap, buffer, &iter,
                               _("Cantonese Pronunciation:"), csp);
    
      csp = mcharmap_get_unicode_kJapaneseOn (uc);
      if (csp)
        insert_vanilla_detail (charmap, buffer, &iter,
                               _("Japanese On Pronunciation:"), csp);
    
      csp = mcharmap_get_unicode_kJapaneseKun (uc);
      if (csp)
        insert_vanilla_detail (charmap, buffer, &iter,
                               _("Japanese Kun Pronunciation:"), csp);
    
      csp = mcharmap_get_unicode_kTang (uc);
      if (csp)
        insert_vanilla_detail (charmap, buffer, &iter,
                               _("Tang Pronunciation:"), csp);
    
      csp = mcharmap_get_unicode_kKorean (uc);
      if (csp)
        insert_vanilla_detail (charmap, buffer, &iter,
...
static void
chartable_status_message (McharmapCharmap *charmap,
                          const gchar *message)
{
  g_signal_emit (charmap, mcharmap_charmap_signals[STATUS_MESSAGE], 
                 0, message);
}
                            ...
                            McharmapCharmap *charmap)
{
  ...
  wc = mcharmap_chartable_get_active_character (priv->chartable);

  /* Forward the notification */
  g_object_notify (G_OBJECT (charmap), "active-character");

  if (priv->active_page == MCHARMAP_CHARMAP_PAGE_DETAILS)
    set_details (charmap, wc);

  gs = g_string_sized_new (256);
  g_string_append_printf (gs, "U+%4.4X %s", wc, 
                          mcharmap_get_unicode_name (wc));

#if ENABLE_UNIHAN
  temp = mcharmap_get_unicode_kDefinition (wc);
  if (temp)
    g_string_append_printf (gs, "   %s", temp);
#endif

  temps = mcharmap_get_nameslist_equals (wc);
  if (temps)
    {
  ...
  temps = mcharmap_get_nameslist_stars (wc);
  if (temps)
    {
      g_string_append_printf (gs, "   • %s", temps[0]);
      for (i = 1;  temps[i];  i++)
...
static void
chartable_sync_font_desc (McharmapChartable *chartable,
                          GParamSpec *pspec,
                          McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;
  ...
  font_desc = mcharmap_chartable_get_font_desc (chartable);
  mcharmap_charmap_set_font_desc_internal (charmap,
...
static void
follow_if_link (McharmapCharmap *charmap,
                GtkTextIter *iter)
{
  McharmapCharmapPrivate *priv = charmap->priv;
  ...
  for (tagp = tags;  tagp != NULL;  tagp = tagp->next)
    {
      ...
      if (uc != (gunichar)(-1)) 
        {
          g_signal_emit (charmap, mcharmap_charmap_signals[LINK_CLICKED], 0, 
                         mcharmap_chartable_get_active_character (priv->chartable),
                         uc);
          mcharmap_charmap_set_active_character (charmap, uc);
          break;
        }
    }
...
static void
set_cursor_if_appropriate (McharmapCharmap *charmap,
                           gint x,
                           gint y)
{
  McharmapCharmapPrivate *priv = charmap->priv;
                      ...
                      McharmapCharmap *charmap)
{
  ...
  if (page_num == MCHARMAP_CHARMAP_PAGE_DETAILS)
    set_details (charmap, mcharmap_chartable_get_active_character (priv->chartable));
  else if (page_num == MCHARMAP_CHARMAP_PAGE_CHARTABLE)
    {
      GtkTextBuffer *buffer;
                                    ...
                                    McharmapCharmap *charmap)
{
  ...
  codepoint_list = mcharmap_chapters_view_get_codepoint_list (priv->chapters_view);
  mcharmap_chartable_set_codepoint_list (priv->chartable, codepoint_list);
...
static void
mcharmap_charmap_init (McharmapCharmap *charmap)
{
  ...
  priv = charmap->priv = G_TYPE_INSTANCE_GET_PRIVATE (charmap, MCHARMAP_TYPE_CHARMAP, McharmapCharmapPrivate);
  ...
  view = mcharmap_chapters_view_new ();
  priv->chapters_view = MCHARMAP_CHAPTERS_VIEW (view);
  ...
  chartable = mcharmap_chartable_new ();
  priv->chartable = MCHARMAP_CHARTABLE (chartable);
  ...
  g_assert (page == MCHARMAP_CHARMAP_PAGE_CHARTABLE);
  gtk_widget_show (scrolled_window);
  ...
  g_assert (page == MCHARMAP_CHARMAP_PAGE_DETAILS);
  gtk_widget_show (scrolled_window);
...
GtkWidget *
mcharmap_charmap_new (void)
{
  return g_object_new (MCHARMAP_TYPE_CHARMAP,
                       "orientation", GTK_ORIENTATION_HORIZONTAL,
                       NULL);
}

#ifndef MCHARMAP_DISABLE_DEPRECATED_SOURCE

/**
 * mcharmap_charmap_set_orientation:
 * @charmap:
 * @orientation:
 *
 * Deprecated: 2.25.0
 */
void
mcharmap_charmap_set_orientation (McharmapCharmap *charmap,
                                   GtkOrientation orientation)
{
#if GTK_CHECK_VERSION (2, 15, 0)
  ...
  g_return_if_fail (MCHARMAP_IS_CHARMAP (charmap));
  priv = charmap->priv;
...
/**
 * mcharmap_charmap_get_orientation:
 * @charmap:
 *
 * Deprecated: 2.25.0
 */
GtkOrientation
mcharmap_charmap_get_orientation (McharmapCharmap *charmap)
{
  g_return_val_if_fail (MCHARMAP_IS_CHARMAP (charmap), GTK_ORIENTATION_HORIZONTAL);
...
#endif /* !MCHARMAP_DISABLE_DEPRECATED_SOURCE */

/**
 * mcharmap_charmap_set_font_desc:
 * @charmap:
 ...
 */
void
mcharmap_charmap_set_font_desc (McharmapCharmap *charmap,
                                 PangoFontDescription *font_desc)
{
  McharmapCharmapPrivate *priv;
...
/**
 * mcharmap_charmap_get_font_desc:
 * @charmap: a #McharmapCharmap
 *
...
mcharmap_charmap_get_font_desc (McharmapCharmap *charmap)
{
  g_return_val_if_fail (MCHARMAP_IS_CHARMAP (charmap), NULL);

  return charmap->priv->font_desc;
}

void
mcharmap_charmap_set_active_character (McharmapCharmap *charmap,
                                        gunichar          wc)
{
  McharmapCharmapPrivate *priv;
  ...
  priv = charmap->priv;
  if (!mcharmap_chapters_view_select_character (priv->chapters_view, wc)) {
    g_warning ("mcharmap_chapters_view_select_character failed (U+%04X)\n", wc);
    return;
  }

  mcharmap_chartable_set_active_character (priv->chartable, wc);
}

gunichar
mcharmap_charmap_get_active_character (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;

  return mcharmap_chartable_get_active_character (priv->chartable);
}

void
mcharmap_charmap_set_active_chapter (McharmapCharmap *charmap,
                                      const gchar *chapter)
{
  McharmapCharmapPrivate *priv = charmap->priv;

  mcharmap_chapters_view_set_selected (priv->chapters_view, chapter);
}

char *
mcharmap_charmap_get_active_chapter (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;

  return mcharmap_chapters_view_get_selected (priv->chapters_view);
}

void
mcharmap_charmap_next_chapter (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;

  mcharmap_chapters_view_next (priv->chapters_view);
}

void
mcharmap_charmap_previous_chapter (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;

  mcharmap_chapters_view_previous (priv->chapters_view);
}

McharmapChartable *
mcharmap_charmap_get_chartable (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;
...
void
mcharmap_charmap_set_chapters_model (McharmapCharmap  *charmap,
                                      McharmapChaptersModel *model)
{
  ...
  mcharmap_chapters_view_set_model (priv->chapters_view, model);
  if (!model) {
    g_object_thaw_notify (object);
    return;
  }
  ...
  if (priv->last_character_set) {
    wc = mcharmap_chartable_get_active_character (priv->chartable);
    mcharmap_charmap_set_active_character (charmap, wc);
  }
...
mcharmap_charmap_get_chapters_model (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;

  return mcharmap_chapters_view_get_model (priv->chapters_view);
}

McharmapChaptersView *
mcharmap_charmap_get_chapters_view  (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;
...
mcharmap_charmap_get_book_codepoint_list (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;

  return mcharmap_chapters_view_get_book_codepoint_list (priv->chapters_view);
}

void
mcharmap_charmap_set_chapters_visible (McharmapCharmap *charmap,
                                        gboolean visible)
{
  McharmapCharmapPrivate *priv = charmap->priv;
...
gboolean
mcharmap_charmap_get_chapters_visible (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;
...
void
mcharmap_charmap_set_page_visible (McharmapCharmap *charmap,
                                    int page,
                                    gboolean visible)
{
  McharmapCharmapPrivate *priv = charmap->priv;
...
gboolean
mcharmap_charmap_get_page_visible (McharmapCharmap *charmap,
                                    int page)
{
  McharmapCharmapPrivate *priv = charmap->priv;
  GtkWidget *page_widget;
...
void
mcharmap_charmap_set_active_page (McharmapCharmap *charmap,
                                   int page)
{
  McharmapCharmapPrivate *priv = charmap->priv;
...
int
mcharmap_charmap_get_active_page (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;
...
McharmapCodepointList *
mcharmap_charmap_get_active_codepoint_list (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;

  return mcharmap_chartable_get_codepoint_list (priv->chartable);
}

/**
 * mcharmap_charmap_set_snap_pow2:
 * @charmap: a #McharmapCharmap
 ...
 */
void
mcharmap_charmap_set_snap_pow2 (McharmapCharmap *charmap,
                                 gboolean snap)
{
  McharmapCharmapPrivate *priv = charmap->priv;

  /* This will cause us to emit our prop notification too */
  mcharmap_chartable_set_snap_pow2 (priv->chartable, snap);
}

/**
 * mcharmap_charmap_get_snap_pow2:
 * @charmap: a #McharmapCharmap
 *
 ...
 */
gboolean
mcharmap_charmap_get_snap_pow2 (McharmapCharmap *charmap)
{
  McharmapCharmapPrivate *priv = charmap->priv;

  return mcharmap_chartable_get_snap_pow2 (priv->chartable);
}
