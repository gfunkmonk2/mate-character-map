...
static void mcharmap_mini_font_selection_class_init (McharmapMiniFontSelectionClass *klass);
static void mcharmap_mini_font_selection_init       (McharmapMiniFontSelection *fontsel);
static void mcharmap_mini_font_selection_finalize   (GObject *object);

G_DEFINE_TYPE (McharmapMiniFontSelection, mcharmap_mini_font_selection, GTK_TYPE_HBOX)

static void
fill_font_families_combo (McharmapMiniFontSelection *fontsel)
{
...
static void
update_font_family_combo (McharmapMiniFontSelection *fontsel)
{
...
family_combo_changed (GtkComboBox *combo,
                      McharmapMiniFontSelection *fontsel)
{
  GtkTreeIter iter;
  char *family;
...
static int
get_font_size (McharmapMiniFontSelection *fontsel)
{
...
/* size is in points */
static void
set_font_size (McharmapMiniFontSelection *fontsel, 
               int size)
{
...
static void
mcharmap_mini_font_selection_finalize (GObject *object)
{
  McharmapMiniFontSelection *fontsel = MCHARMAP_MINI_FONT_SELECTION (object);
  pango_font_description_free (fontsel->font_desc);

  G_OBJECT_CLASS (mcharmap_mini_font_selection_parent_class)->finalize (object);
}

static void
mcharmap_mini_font_selection_set_property (GObject *object,
                                            guint prop_id,
                                            const GValue *value,
                                            GParamSpec *pspec)
{
  McharmapMiniFontSelection *mini_fontsel = MCHARMAP_MINI_FONT_SELECTION (object);

  switch (prop_id) {
    case PROP_FONT_DESC:
      mcharmap_mini_font_selection_set_font_desc (mini_fontsel, g_value_get_boxed (value));
      break;
    default:
...
static void
mcharmap_mini_font_selection_get_property (GObject *object,
                                            guint prop_id,
                                            GValue *value,
                                            GParamSpec *pspec)
{
  ...
  switch (prop_id) {
    case PROP_FONT_DESC:
      g_value_set_boxed (value, mcharmap_mini_font_selection_get_font_desc (mini_fontsel));
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
static void
mcharmap_mini_font_selection_class_init (McharmapMiniFontSelectionClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);

  gobject_class->finalize = mcharmap_mini_font_selection_finalize;
  gobject_class->get_property = mcharmap_mini_font_selection_get_property;
  gobject_class->set_property = mcharmap_mini_font_selection_set_property;
...
GtkWidget *
mcharmap_mini_font_selection_new (void)
{
  return GTK_WIDGET (g_object_new (mcharmap_mini_font_selection_get_type (), 
                                   NULL));
}

void
mcharmap_mini_font_selection_set_font_desc (McharmapMiniFontSelection *fontsel,
                                             PangoFontDescription *font_desc)
{
  ...
  g_return_if_fail (MCHARMAP_IS_MINI_FONT_SELECTION (fontsel));
...
mcharmap_mini_font_selection_get_font_desc (McharmapMiniFontSelection *fontsel)
{
  g_return_val_if_fail (MCHARMAP_IS_MINI_FONT_SELECTION (fontsel), NULL);

  return fontsel->font_desc;
}

void
mcharmap_mini_font_selection_change_font_size (McharmapMiniFontSelection *fontsel,
                                                float factor)
{
  int size, new_size;
...
void
mcharmap_mini_font_selection_reset_font_size (McharmapMiniFontSelection *fontsel)
{
