...
#define MCHARMAP_PRINT_OPERATION_GET_PRIVATE(print_operation)(G_TYPE_INSTANCE_GET_PRIVATE ((print_operation), MCHARMAP_TYPE_PRINT_OPERATION, McharmapPrintOperationPrivate))

#define GRID_LINE_WIDTH (0.25)

struct _McharmapPrintOperationPrivate
{
...
G_DEFINE_TYPE (McharmapPrintOperation, mcharmap_print_operation, GTK_TYPE_PRINT_OPERATION)

/* helper functions */

static void
draw_character_cell (McharmapPrintOperation *print_operation,
                     cairo_t *cr,
                     int row,
                     int col,
                     int character_index)
{
  McharmapPrintOperationPrivate *priv = print_operation->priv;
  gunichar wc;
  char utf8[7];
  int len;
  double x, y;
  ...
  wc = mcharmap_codepoint_list_get_char (priv->codepoint_list, character_index);
...
static void
draw_cell_grid (McharmapPrintOperation *print_operation,
                cairo_t *cr,
                int last_row,
                int last_col)
{
...
static void
mcharmap_print_operation_begin_print (GtkPrintOperation *operation,
                                       GtkPrintContext   *context)
{
  McharmapPrintOperation *print_operation = MCHARMAP_PRINT_OPERATION (operation);
  McharmapPrintOperationPrivate *priv = print_operation->priv;
  PangoFontDescription *info_font_desc;
  double width, height;
  int page_size, pages;

  priv->last_codepoint_index = mcharmap_codepoint_list_get_last_index (priv->codepoint_list);
...
static void
mcharmap_print_operation_draw_page (GtkPrintOperation *operation,
                                     GtkPrintContext   *context,
                                     int                page_nr)
{
  McharmapPrintOperation *print_operation = MCHARMAP_PRINT_OPERATION (operation);
...
static void
mcharmap_print_operation_end_print (GtkPrintOperation *operation,
                                     GtkPrintContext   *context)
{
  McharmapPrintOperation *print_operation = MCHARMAP_PRINT_OPERATION (operation);
  McharmapPrintOperationPrivate *priv = print_operation->priv;
...
static GtkWidget *
mcharmap_print_operation_create_custom_widget (GtkPrintOperation *operation)
{
/*  McharmapPrintOperation *print_operation = MCHARMAP_PRINT_OPERATION (operation);
  McharmapPrintOperationPrivate *priv = print_operation->priv;*/
  return GTK_PRINT_OPERATION_CLASS (mcharmap_print_operation_parent_class)->create_custom_widget (operation);
}

static void
mcharmap_print_operation_custom_widget_apply (GtkPrintOperation *operation,
                                               GtkWidget *widget)
{
/*  McharmapPrintOperation *print_operation = MCHARMAP_PRINT_OPERATION (operation);
  McharmapPrintOperationPrivate *priv = print_operation->priv;*/
...
static void
mcharmap_print_operation_init (McharmapPrintOperation *print_operation)
{
  McharmapPrintOperationPrivate *priv;

  priv = print_operation->priv = MCHARMAP_PRINT_OPERATION_GET_PRIVATE (print_operation);
}

static GObject *
mcharmap_print_operation_constructor (GType type,
                                       guint n_construct_properties,
                                       GObjectConstructParam *construct_params)
{
  GObject *object;
  McharmapPrintOperation *print_operation;
  McharmapPrintOperationPrivate *priv;

  object = G_OBJECT_CLASS (mcharmap_print_operation_parent_class)->constructor
             (type, n_construct_properties, construct_params);

  print_operation = MCHARMAP_PRINT_OPERATION (object);
...
static void
mcharmap_print_operation_finalize (GObject *object)
{
//   McharmapPrintOperation *print_operation = MCHARMAP_PRINT_OPERATION (object);
//   McharmapPrintOperationPrivate *priv = print_operation->priv;

  G_OBJECT_CLASS (mcharmap_print_operation_parent_class)->finalize (object);
}

static void
mcharmap_print_operation_set_property (GObject *object,
			   guint prop_id,
			   const GValue *value,
			   GParamSpec *pspec)
{
...
static void
mcharmap_print_operation_class_init (McharmapPrintOperationClass *klass)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS (klass);
  GtkPrintOperationClass *print_op_class = GTK_PRINT_OPERATION_CLASS (klass);

  gobject_class->constructor = mcharmap_print_operation_constructor;
  gobject_class->finalize = mcharmap_print_operation_finalize;
  gobject_class->set_property = mcharmap_print_operation_set_property;

  print_op_class->begin_print = mcharmap_print_operation_begin_print;
  print_op_class->draw_page = mcharmap_print_operation_draw_page;
  print_op_class->end_print = mcharmap_print_operation_end_print;
  print_op_class->create_custom_widget = mcharmap_print_operation_create_custom_widget;
  print_op_class->custom_widget_apply = mcharmap_print_operation_custom_widget_apply;
  ...
  g_type_class_add_private (gobject_class, sizeof (McharmapPrintOperationPrivate));
}

/* public API */

GtkPrintOperation *
mcharmap_print_operation_new (McharmapCodepointList *codepoint_list,
                               PangoFontDescription *font_desc)
{
  return g_object_new (MCHARMAP_TYPE_PRINT_OPERATION,
