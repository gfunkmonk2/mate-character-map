...
#define MCHARMAP_SEARCH_DIALOG_GET_PRIVATE(o) (G_TYPE_INSTANCE_GET_PRIVATE ((o), mcharmap_search_dialog_get_type (), McharmapSearchDialogPrivate))
...
static guint mcharmap_search_dialog_signals[NUM_SIGNALS];

enum
{
  MCHARMAP_RESPONSE_PREVIOUS,
  MCHARMAP_RESPONSE_NEXT
};

typedef struct _McharmapSearchDialogPrivate McharmapSearchDialogPrivate;
typedef struct _McharmapSearchState McharmapSearchState;

struct _McharmapSearchState
{
...
struct _McharmapSearchDialogPrivate
{
  McharmapWindow       *guw;
  GtkWidget             *entry;
  GtkWidget             *whole_word_option;
  GtkWidget             *annotations_option;
  McharmapSearchState  *search_state;
  GtkWidget             *prev_button;
  GtkWidget             *next_button;
};

static void mcharmap_search_dialog_class_init (McharmapSearchDialogClass *klass);
static void mcharmap_search_dialog_init       (McharmapSearchDialog *dialog);

G_DEFINE_TYPE (McharmapSearchDialog, mcharmap_search_dialog, GTK_TYPE_DIALOG)
...
static gboolean
matches (McharmapSearchDialog *search_dialog,
         gunichar               wc,
         const gchar           *search_string_nfd,
         const gboolean         annotations)
{
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);
  ...
  haystack = mcharmap_get_unicode_data_name (wc);
  if (haystack)
    {
  ...
  if (annotations)
...
#if ENABLE_UNIHAN
      haystack = mcharmap_get_unicode_kDefinition (wc);
      if (haystack)
	{
	  priv->search_state->strings_checked++;
	...
	return TRUE;
      ...
      haystack_arr = mcharmap_get_nameslist_equals (wc);
      if (haystack_arr)
	{
	...
	return TRUE;
      ...
      haystack_arr = mcharmap_get_nameslist_stars (wc);
      if (haystack_arr)
	{
	  for (i = 0; haystack_arr[i] != NULL; i++)
	    {
	...
	return TRUE;
      ...
      haystack_arr = mcharmap_get_nameslist_colons (wc);
      if (haystack_arr)
	{
	  for (i = 0; haystack_arr[i] != NULL; i++)
	    {
	...
	return TRUE;
      ...
      haystack_arr = mcharmap_get_nameslist_pounds (wc);
      if (haystack_arr)
	{
	  for (i = 0; haystack_arr[i] != NULL; i++)
	    {
...
static gint
check_for_explicit_codepoint (const McharmapCodepointList *list,
                              const gchar                  *string)
{
  const gchar *nptr;
  gchar *endptr;
  ...
  if (nptr != string)
    {
      ...
      if (endptr != nptr)
        {
          gint index = mcharmap_codepoint_list_get_index ((McharmapCodepointList *) list, wc);
          if (index != -1)
            return index;
        }
    }
...
static gboolean
quick_checks_before (McharmapSearchState *search_state)
{
...
static gboolean
quick_checks_after (McharmapSearchState *search_state)
{
  ...
  if (!search_state->whole_word)
    if (search_state->search_string_nfd[0] < 0x20 || search_state->search_string_nfd[0] > 0x7e)
      {
	gint index = mcharmap_codepoint_list_get_index (search_state->list, g_utf8_get_char (search_state->search_string_nfd));
	if (index != -1)
	  {
...
static gboolean
idle_search (McharmapSearchDialog *search_dialog)
{
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);
  gunichar wc;
  GTimer *timer;
  ...
  do
    {
      ...
      wc = mcharmap_codepoint_list_get_char (priv->search_state->list, priv->search_state->curr_index);

      if (!mcharmap_unichar_validate (wc) || !mcharmap_unichar_isdefined (wc))
        continue;
...
/**
 * mcharmap_search_state_get_found_char:
 * @search_state: 
 * Return value: 
 **/
static gunichar
mcharmap_search_state_get_found_char (McharmapSearchState *search_state)
{
  if (search_state->found_index > 0)
    return mcharmap_codepoint_list_get_char (search_state->list, search_state->found_index);
  else
    return (gunichar)(-1);
}

/**
 * mcharmap_search_state_free:
 * @search_state: 
 **/
static void
mcharmap_search_state_free (McharmapSearchState *search_state)
{
  g_object_unref (search_state->list);
  g_free (search_state->search_string_nfd_temp);
  g_free (search_state->search_string_nfc);
  g_slice_free (McharmapSearchState, search_state);
}

/**
 * mcharmap_search_state_new:
 * @list: a #McharmapCodepointList to be searched
 ...
 *
 * Initializes a #McharmapSearchState to search for the next character in
 * the codepoint list that matches @search_string. Assumes input is valid.
 *
 * Return value: the new #McharmapSearchState.
 **/
static McharmapSearchState * 
mcharmap_search_state_new (McharmapCodepointList       *list,
                            ...
                            gboolean                      annotations)
{
  ...
  g_assert (direction == MCHARMAP_DIRECTION_BACKWARD || direction == MCHARMAP_DIRECTION_FORWARD);

  search_state = g_slice_new (McharmapSearchState);

  search_state->list = g_object_ref (list);
  search_state->list_num_chars = mcharmap_codepoint_list_get_last_index (search_state->list) + 1;
  ...
  if (search_state->search_string_nfd_len == 1)
    search_state->search_index_nfd = mcharmap_codepoint_list_get_index (search_state->list, g_utf8_get_char (search_state->search_string_nfd));
  else
  ...
  if (search_state->search_string_nfc_len == 1)
    search_state->search_index_nfc = mcharmap_codepoint_list_get_index (search_state->list, g_utf8_get_char (search_state->search_string_nfc));
  else
...
static void
information_dialog (McharmapSearchDialog *search_dialog,
                    const gchar           *message)
{
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);
  GtkWidget *dialog;
  ...
  gtk_window_set_icon_name (GTK_WINDOW (dialog), MCHARMAP_ICON_NAME);
...
static void
search_completed (McharmapSearchDialog *search_dialog)
{
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);
  gunichar found_char = mcharmap_search_state_get_found_char (priv->search_state);

  priv->search_state->searching = FALSE;

  g_signal_emit (search_dialog, mcharmap_search_dialog_signals[SEARCH_FINISH], 0, found_char);
...
static void
_mcharmap_search_dialog_fire_search (McharmapSearchDialog *search_dialog,
                                      McharmapDirection     direction)
{
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);
  McharmapCodepointList *list;
  ...
  GdkCursor *cursor = _mcharmap_window_progress_cursor ();
  gdk_window_set_cursor (gtk_widget_get_window (GTK_WIDGET (search_dialog)), cursor);
  gdk_cursor_unref (cursor);

  list = mcharmap_charmap_get_book_codepoint_list (priv->guw->charmap);
  if (!list)
    return;
  ...
  if (priv->search_state == NULL
      ...
      || priv->search_state->annotations != gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (priv->annotations_option)) )
    {
      if (priv->search_state)
        mcharmap_search_state_free (priv->search_state);

      start_char = mcharmap_charmap_get_active_character (priv->guw->charmap);
      start_index = mcharmap_codepoint_list_get_index (list, start_char);
      priv->search_state = mcharmap_search_state_new (list, gtk_entry_get_text (GTK_ENTRY (priv->entry)),
    ...
    {
      start_char = mcharmap_charmap_get_active_character (priv->guw->charmap);
      priv->search_state->start_index = mcharmap_codepoint_list_get_index (list, start_char);
  ...
  g_signal_emit (search_dialog, mcharmap_search_dialog_signals[SEARCH_START], 0);

  g_object_unref (list);
}

void
mcharmap_search_dialog_start_search (McharmapSearchDialog *search_dialog,
                                      McharmapDirection     direction)
{
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);

  if (priv->search_state != NULL && !_entry_is_empty (GTK_ENTRY (priv->entry)))
    _mcharmap_search_dialog_fire_search (search_dialog, direction);
  else
...
search_find_response (GtkDialog *dialog,
                      gint       response)
{
  McharmapSearchDialog *search_dialog = MCHARMAP_SEARCH_DIALOG (dialog);
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);

  switch (response)
    {
      case MCHARMAP_RESPONSE_PREVIOUS:
        _mcharmap_search_dialog_fire_search (search_dialog, MCHARMAP_DIRECTION_BACKWARD);
        break;

      case MCHARMAP_RESPONSE_NEXT:
        _mcharmap_search_dialog_fire_search (search_dialog, MCHARMAP_DIRECTION_FORWARD);
        break;
...
entry_changed (GtkObject             *object,
               McharmapSearchDialog *search_dialog)
{
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);
  gboolean is_empty;
...
static void
mcharmap_search_dialog_init (McharmapSearchDialog *search_dialog)
{
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);
  ...
  gtk_dialog_add_action_widget (GTK_DIALOG (search_dialog), priv->prev_button, MCHARMAP_RESPONSE_PREVIOUS);
  ...
  gtk_dialog_add_action_widget (GTK_DIALOG (search_dialog), priv->next_button, MCHARMAP_RESPONSE_NEXT);

  gtk_dialog_set_default_response (GTK_DIALOG (search_dialog), MCHARMAP_RESPONSE_NEXT);
...
static void 
mcharmap_search_dialog_finalize (GObject *object)
{
  McharmapSearchDialog *search_dialog = MCHARMAP_SEARCH_DIALOG (object);
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);

  if (priv->search_state)
    mcharmap_search_state_free (priv->search_state);

  G_OBJECT_CLASS (mcharmap_search_dialog_parent_class)->finalize (object);
}

static void
mcharmap_search_dialog_class_init (McharmapSearchDialogClass *clazz)
{
  g_type_class_add_private (clazz, sizeof (McharmapSearchDialogPrivate));

  G_OBJECT_CLASS (clazz)->finalize = mcharmap_search_dialog_finalize;

  clazz->search_start = NULL;
  clazz->search_finish = NULL;

  mcharmap_search_dialog_signals[SEARCH_START] =
      g_signal_new (I_("search-start"), mcharmap_search_dialog_get_type (), G_SIGNAL_RUN_FIRST,
                    G_STRUCT_OFFSET (McharmapSearchDialogClass, search_start), NULL, NULL, 
                    g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  mcharmap_search_dialog_signals[SEARCH_FINISH] =
      g_signal_new (I_("search-finish"), mcharmap_search_dialog_get_type (), G_SIGNAL_RUN_FIRST, 
                    G_STRUCT_OFFSET (McharmapSearchDialogClass, search_finish), NULL, NULL, 
                    g_cclosure_marshal_VOID__UINT, G_TYPE_NONE, 1, G_TYPE_UINT);
}

GtkWidget *
mcharmap_search_dialog_new (McharmapWindow *guw)
{
  McharmapSearchDialog *search_dialog = g_object_new (mcharmap_search_dialog_get_type (), NULL);
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);
...
void
mcharmap_search_dialog_present (McharmapSearchDialog *search_dialog)
{
  gtk_widget_grab_focus (MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog)->entry);
  gtk_window_present (GTK_WINDOW (search_dialog));
}

gdouble
mcharmap_search_dialog_get_completed (McharmapSearchDialog *search_dialog)
{
  McharmapSearchDialogPrivate *priv = MCHARMAP_SEARCH_DIALOG_GET_PRIVATE (search_dialog);
  ...
  if (priv->search_state == NULL || !priv->search_state->searching)
    ...
    {
#if ENABLE_UNIHAN
      gdouble total = mcharmap_get_unicode_data_name_count () + mcharmap_get_unihan_count ();
#else
      gdouble total = mcharmap_get_unicode_data_name_count ();
#endif
