...
const gchar* mcharmap_get_unicode_name(gunichar wc)
{
	static gchar buf[32];
	...
	* instead of via mcharmap_get_unicode_data_name below, to save some space?
	*/
	...
	{
		const gchar* x = mcharmap_get_unicode_data_name(wc);
...
const gchar* mcharmap_get_unicode_category_name(gunichar wc)
{
	_mcharmap_intl_ensure_initialized();

	switch (mcharmap_unichar_type(wc))
    {
...
McharmapUnicodeVersion mcharmap_get_unicode_version(gunichar uc)
{
	gint min = 0;
	gint mid;
	...
	{
		return MCHARMAP_UNICODE_VERSION_UNASSIGNED;
	}
...
{
	g_return_val_if_fail(version >= MCHARMAP_UNICODE_VERSION_UNASSIGNED && version <= MCHARMAP_UNICODE_VERSION_LATEST, NULL);

	if (G_UNLIKELY(version == MCHARMAP_UNICODE_VERSION_UNASSIGNED))
	{
		return NULL;
	}
...
gint mcharmap_get_unihan_count()
{
	return 0;
}
...
_mcharmap_unicode_has_nameslist_entry (gunichar uc)
{
...
gunichar *
mcharmap_get_nameslist_exes (gunichar uc)
{
  const NamesList *nl;
  gunichar *exes;
  gunichar i, count;
...
const gchar** mcharmap_get_nameslist_equals(gunichar uc)
{
	const NamesList* nl;
	const gchar** equals;
	gunichar i, count;
...
/* returns newly allocated null-terminated array of gchar* */
/* the items are const, but the array should be freed by the caller */
const gchar** mcharmap_get_nameslist_stars(gunichar uc)
{
	const NamesList* nl;
	const gchar** stars;
	gunichar i, count;
...
/* returns newly allocated null-terminated array of gchar* */
/* the items are const, but the array should be freed by the caller */
const gchar** mcharmap_get_nameslist_pounds(gunichar uc)
{
	const NamesList* nl;
	const gchar** pounds;
	gunichar i, count;
...
/* returns newly allocated null-terminated array of gchar* */
/* the items are const, but the array should be freed by the caller */
const gchar** mcharmap_get_nameslist_colons(gunichar uc)
{
	const NamesList* nl;
	const gchar** colons;
	gunichar i, count;
...
/**
 * mcharmap_unichar_to_printable_utf8
 ...
 **/
gint
mcharmap_unichar_to_printable_utf8 (gunichar uc, gchar *outbuf)
{
  ...
  if (! mcharmap_unichar_validate (uc) || (! mcharmap_unichar_isgraph (uc)
      && mcharmap_unichar_type (uc) != G_UNICODE_PRIVATE_USE))
    return 0;
  else if (mcharmap_unichar_type (uc) == G_UNICODE_COMBINING_MARK
      || mcharmap_unichar_type (uc) == G_UNICODE_ENCLOSING_MARK
      || mcharmap_unichar_type (uc) == G_UNICODE_NON_SPACING_MARK)
    {
      gint x;
 ...
 **/
GUnicodeType
mcharmap_unichar_type (gunichar uc)
{
  gint min = 0;
  gint mid;
 ...
 **/
gboolean
mcharmap_unichar_isdefined (gunichar uc)
{
  return mcharmap_unichar_type (uc) != G_UNICODE_UNASSIGNED;
}

/**
 * mcharmap_unichar_isgraph:
 * @uc: a Unicode character
 *
 ...
 **/
gboolean
mcharmap_unichar_isgraph (gunichar uc)
{
  GUnicodeType t = mcharmap_unichar_type (uc);
 ...
 */
gunichar
mcharmap_unicode_get_locale_character (void)
{
  GtkStockItem item;
